...
def get_job_status(job_id):
    ...
    Base on a GET request, it retrieves the status of the Kaniko job and the results if the job has completed.
    '''
    if CHASSIS_DEV:
        # if you are doing local dev you need to point at the local kubernetes cluster with your config file
        kubefile = os.getenv("CHASSIS_KUBECONFIG")
...
def build_image():
    ...
    It generates a model image based upon a POST request. The `request.files` structure can be seen in [Python SDK](../docs/docs/chassisml-sdk-reference.md#usage)
    '''
