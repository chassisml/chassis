...
func BuildUrlRoute(route string, dstBaseUrl ...string) string {
	bUrl := baseUrl
	if len(dstBaseUrl) > 0 {
		bUrl = dstBaseUrl[0]
	}
...
func BuildKFServingRedirect(fullPath string) string {
	return fmt.Sprintf("http://127.0.0.1:%s%s", kfservingPort, fullPath)
}
